...
def check_lock(user_name):                               # ##检查帐号是否加锁
    with open(setting.PATH_SHOP_LOCK, 'r') as lock:                    # #读加锁帐号文件信息
...
def check_user(user_name, pass_word):               # ##检查帐号/密码是否正确
    global INPUT
    with open(setting.PATH_SHOP_USER, 'r') as User_Pass:         # #读帐号密码文件信息
...
def lock_user(user_name):                 # ##锁定帐号
    with open(setting.PATH_SHOP_LOCK, 'a') as f:
        f.write('%s\n ' % user_name)
    return True
...
def atm_login(func):
    ...
    def inner(*args, **kwargs):
        if not login_in.USER_STATUS['username']:
            print('当前未登陆ATM帐号'.center(60, '*'))
            print('即将跳转至ATM用户登录')
            time.sleep(2)
            login_in.login_in()
...
def save_shopping_data(shopping_car, username, salary):
    if len(shopping_car) < 1:                    # 购物车无宝贝,不做保存
        ...
        with open(setting.PATH_SHOP_DATA, 'wb') as f:                            # 保存文件
            pickle.dump(SHOPPING_CAR_HIS, f)


@atm_login
def pay_money(salary):    # 余额充值
    pay_fee, succ_no = atm_user.pay_fee()              # 调用ATM消费模块
    if succ_no:
        salary += pay_fee             # 充值
        time.sleep(1)
        print('已到帐')
...
def read_his_msg(username):
    ...
    with open(setting.PATH_SHOP_DATA, 'rb') as f:  # 读文件
        if len(f.read()) < 1:
            print('无历史购买记录'.center(50, '-'))
        else:
            SHOPPING_CAR_HIS = pickle.load(open(setting.PATH_SHOP_DATA, 'rb'))
                    ...
                    while salary_flag is not True:                                         # 余额不足，一直循环检查余额
                        if price_sum > int(salary):                                       # 余额不足
                            ...
                            if choice_pay == 'Y' or choice_pay == 'y':             # 充值调用充值函数
                                ...
                                print('当前余额为：{} 宝贝价格为：{}'.format(salary, price_sum))
